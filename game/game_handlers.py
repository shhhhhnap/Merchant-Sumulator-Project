    ...
    if game_state.selected_product and game_state.selected_recipient:
        if game_state.selected_product and game_state.selected_recipient:
            ...
            if random.random() > risk:
                # Рассчитываем цену с учётом риска
                price = calculate_sell_price(
                    game_state.selected_product.name,
                    game_state.selected_recipient.region,
                    distance=game_state.selected_recipient.distance
                )
                game_state.sent_products.append(
                    (game_state.selected_product, game_state.selected_recipient,
                     game_state.selected_recipient.distance, price)   # добавили цену
                )
    ...
    if card.state == CardState.BACK:
        if not game_state.waiting_for_action and not game_state.has_revealed_card_this_turn:
            ...
            if card.card_type == CardType.PRODUCT:
                ...
                # Событие: ждём нажатия "принять"
                pass
            elif card.card_type == CardType.NEGATIVE:
                # Негатив: ждём нажатия "принять"
                pass
            ...
            print(f"[DEBUG] Card {index} revealed: {card.name} (type: {card.card_type})")
            return

    # ===== ДЕЙСТВИЕ С ОТКРЫТОЙ КАРТОЙ =====
    if card.state == CardState.REVEALED and game_state.waiting_for_action and index == game_state.current_open_card_index:
        
        buy_rect = pygame.Rect(card.x + 16, card.y + 190, 65, 35)
        skip_rect = pygame.Rect(card.x + 100, card.y + 190, 70, 38)
        accept_rect = pygame.Rect(card.x + 16, card.y + 190, 150, 38)
...
def handle_click(pos):
    ...
    if game_state.current_window == Window.MAIN:
        ...
        for i in range(len(game_state.inventory)):
            ...
            inv_rect = pygame.Rect(x, y, 90, 138)
